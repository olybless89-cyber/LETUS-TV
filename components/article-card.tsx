...
export function ArticleCard({ article, variant = "default" }: Props) {
  if (variant === "paired") {
    return (
      <Link href={`/articles/${article.slug}`} className="group flex gap-4 py-4 border-b border-line last:border-0">
        <div className="relative h-24 w-32 shrink-0 overflow-hidden bg-line">
        ...
        <div className="min-w-0">
          ...
          <h3 className="mt-1 font-display text-base font-bold leading-snug text-ink group-hover:text-blue">
            {article.title}
          </h3>
  ...
  return (
    <Link href={`/articles/${article.slug}`} className="group block">
      ...
      <div className="pt-3">
        ...
        <h3 className="mt-1 font-display text-lg font-bold leading-snug text-ink group-hover:text-blue">
          {article.title}
        </h3>
        {article.excerpt && <p className="mt-1 line-clamp-2 text-sm text-ink-soft">{article.excerpt}</p>}
